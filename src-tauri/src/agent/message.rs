...
/// - Las notas/errores del harness van como `system`, no como `user`: así el modelo deja de
///   percibir que «el usuario lo regaña».
///
...
/// Estimación conservadora de tokens del contenido de un mensaje (~3 chars/token; el código
/// y el JSON densos tokenizan más fino, así que tiende a subestimar — se compensa con el
/// `+4` de overhead por mensaje en quien la usa).
